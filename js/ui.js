...
export const DEFAULT_IMAGE_URL =
  "https://via.placeholder.com/400x250/ecf0f1/7f8c8d?text=Recipe+Image";
...
/**
 * Updates the old icon button if it exists.
 * (Kept safe so code doesn't break even though we now use the animated slider.)
 */
function updateThemeToggleIcon(isDarkMode) {
  const toggleButton = document.getElementById("theme-toggle");
  if (!toggleButton) return; // no old button anymore
...
function createRecipeCard(recipe) {
  ...
  return `
        <div class="recipe-card ${pinnedClass}" data-id="${
    recipe.id
  }" onclick="RecipeApp.showRecipeDetail(${recipe.id})">
            <div class="card-image-container">
                <img src="${imageUrl}" alt="${
    recipe.title
  }" class="card-image" onerror="this.src='${DEFAULT_IMAGE_URL}';">
            </div>
            <div class="card-content">
                <h3>${recipe.title}</h3>
                <p>${recipe.description.substring(0, 80)}${
    recipe.description.length > 80 ? "..." : ""
                ...
                <div class="card-footer">
                    ...
                    <span class="type-tag type-${recipe.type}">${
    recipe.type
  }</span>
                    <div class="card-time">
...
export function renderRecipeList(recipeArray) {
  ...
  if (recipeArray.length === 0) {
    listElement.innerHTML =
      '<p style="grid-column: 1 / -1; text-align: center; padding: 40px; color: #7f8c8d;">No recipes found matching your criteria. Try adjusting the search or filter settings.</p>';
    return;
  }
    ...
    recipe.type
                ...
                <div class="detail-metadata">
                    ...
                    <div class="detail-actions"> 
                        <button class="pin-button ${pinClass}" onclick="RecipeApp.togglePin(${
    recipe.id
  })">
                            <i class="fas fa-thumbtack"></i> ${pinText} 
                        </button>
                        <button class="edit-button" onclick="RecipeApp.editRecipe(${
                          recipe.id
                        })">
                            <i class="fas fa-edit"></i> Edit Recipe
                        </button>
                        <button class="delete-button" onclick="RecipeApp.deleteRecipe(${
                          recipe.id
                        })">
...
/**
 * Syncs the animated theme slider with the current theme
 * and wires up its change listener to RecipeApp.toggleTheme().
 */
export function syncThemeSlider() {
  ...
  // When the slider changes, toggle theme via global RecipeApp
  slider.addEventListener("change", () => {
    if (
      window.RecipeApp &&
      typeof window.RecipeApp.toggleTheme === "function"
    ) {
      window.RecipeApp.toggleTheme();
    }
  });
}
